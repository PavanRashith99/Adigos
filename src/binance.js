...
module.exports = class Binance {
     ...
     */
    exchangeInfo() {
        return this.request('/exchangedetail')
    }
    ...
    klines(symbol, interval, startTime, endTime, limit) {
        return this.request('/charthistory', { qs: { symbol, interval, startTime, endTime, limit } })
    }
    ...
    request(path, options) {
        return new Promise((resolve, reject) => {
            request('http://64.227.146.105:3000' + path, options, (err, res, body) => {
                if (err) {
                    return reject(err)
                }
                if (!body) {
